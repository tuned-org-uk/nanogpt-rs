...
fn test_greedy_sampling() {
    ...
    let ids = sampled.to_data().to_vec::<i32>().unwrap();
    assert_eq!(ids[0], 4);
    assert_eq!(ids[1], 0);
}
...
fn test_top_k_one_equals_greedy() {
    ...
    let top1_ids = top1_sample.to_data().to_vec::<i32>().unwrap();
    let greedy_ids = greedy_sample.to_data().to_vec::<i32>().unwrap();
...
fn test_temperature_zero_is_greedy() {
    ...
    let temp_ids = temp_zero.to_data().to_vec::<i32>().unwrap();
    let greedy_ids = greedy.to_data().to_vec::<i32>().unwrap();
